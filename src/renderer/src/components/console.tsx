import { Box, Checkbox, Link, TextField, Tooltip, Typography } from '@mui/material'
...
const Console = ({
  ...
  const initializeLeetcode = async () => {
    const [submissions, status] = await getLatestLeetcodeSubmissions(username)
    if (submissions === undefined) {
      if (status >= 300) {
        toast(`Leetcode API error while fetching updates. Status code: ${status}`, {
          position: 'top-right',
          style: {
  ...
  const updateLeetcode = async () => {
    ...
    if (newSubmissions.length > 0) {
      ...
      if (failedSubmissions.length > 0 && isDeathlinkEnabled.current) {
        ...
        client?.deathLink.sendDeathLink(slot, `Failed a Leetcode submission: ${cause}`)
        client?.messages.say(`Failed a Leetcode submission: ${failedTitle} (${cause})`)
      }
    }

    // update submission history
    submissionHistory.current = [...submissionHistory.current, ...newSubmissions]
  }

  useEffect(() => {
    initializeLeetcode()
    const interval = setInterval(updateLeetcode, 5000)
  ...
  return (
    <Box width="100%" display="flex" flexDirection="column" gap={1} alignItems="center">
      ...
      <Typography>
        Connected to{' '}
        <Tooltip arrow title={<Typography>Slot: {slot}</Typography>} placement="top">
      ...
      >
        {messageHistory.map((text: string, idx: number) =>
          idx === messageHistory.length - 1 ? (
            ...
            <Typography alignSelf="flex-start" key={`message-${idx}`} whiteSpace="pre-wrap">
              <code>{text}</code>
            </Typography>
          )
        )}
      ...
      <Box display="flex" alignItems="center">
        ...
        <Tooltip
          title={
            <Typography>
              If checked, failed submissions will trigger a death for all players with DeathLink
              enabled.
            </Typography>
          }
          arrow
